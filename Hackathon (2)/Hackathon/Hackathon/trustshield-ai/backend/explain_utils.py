...
def rule_engine(email_text):
    ...
    urgency_words = ["urgent", "immediately", "act now", "within 24 hours", "deadline", "asap", "avoid interruption"]
    fear_words = ["account suspended", "service suspension", "legal action", "lawsuit", "breach detected", "mailbox is almost full"]
    authority_words = ["ceo", "hr", "finance department", "manager request"]
    credential_words = [
        "verify your login", "restore access", "account suspended", "login credentials",
        "revalidate your account", "update credentials", "confirm your identity", "kyc",
    ]
    payment_words = ["invoice", "confirm payment details", "wire transfer", "gift card"]
    ...
    if any(word in text for word in payment_words):
        boost += 0.12
        triggered.append("Payment fraud pattern detected")
