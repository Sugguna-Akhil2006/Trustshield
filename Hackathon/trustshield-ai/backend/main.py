...
def load_resources() -> None:
    ...
    if not MONGO_URI:
        ...
        try:
            ...
            try:
                existing_indexes = email_collection.index_information()
                text_hash_index = existing_indexes.get("text_hash_1")
                if text_hash_index and text_hash_index.get("unique"):
                    email_collection.drop_index("text_hash_1")
                ...
                email_collection.create_index("threat_id", unique=False)
    ...
    _sync_live_model_state_from_metadata()
...
def rule_based_score(text: str, urls: list[str]) -> Dict[str, int | bool]:
    ...
    credential_terms = [
        ...
        "update credentials", "revalidate your account", "confirm your identity",
        "verify mailbox", "mailbox verification", "kyc", "verify now",
        "confirm payment details",
    ]
    threat_terms = [
        "mailbox is almost full", "service suspension", "account suspension",
        "account will be suspended", "unusual sign-in", "suspicious login activity",
        "security alert", "avoid interruption", "prevent suspension",
        "kyc is pending", "pending kyc",
    ]
    payment_fraud_terms = [
        "confirm payment details", "invoice attached", "pending payment",
        "payment required immediately", "wire transfer", "gift card",
    ]
    ...
    payment_fraud_hits = sum(1 for term in payment_fraud_terms if term in text)
    ...
    category_hits = (
        int(urgency_hits > 0) +
        int(credential_hits > 0) +
        int(suspicious_url_hits > 0) +
        int(threat_hits > 0) +
        int(payment_fraud_hits > 0)
    )
    strong_signal = (
        (urgency_hits >= 1 and credential_hits >= 1 and suspicious_url_hits >= 1) or
        (credential_hits >= 2 and suspicious_url_hits >= 2) or
        (urgency_hits >= 2 and credential_hits >= 2) or
        (credential_hits >= 1 and threat_hits >= 1) or
        (payment_fraud_hits >= 1 and credential_hits >= 1) or
        (threat_hits >= 2)
    )
    ...
    return {
        ...
        "payment_fraud_hits": payment_fraud_hits,
...
async def analyze_text(full_text: str) -> Dict[str, Any]:
    ...
    threat_hits = int(rule_result.get("threat_hits", 0))
    payment_fraud_hits = int(rule_result.get("payment_fraud_hits", 0))
    credential_hits = int(rule_result.get("credential_hits", 0))
    urgency_hits = int(rule_result.get("urgency_hits", 0))
    ...
    if job_offer_strong_signal:
        ...
        risk_level = "High" if final_score < 90 else "Critical"
        trust_score = round(100 - final_score, 2)
    elif rule_score >= 5 and threat_hits >= 1 and (credential_hits >= 1 or urgency_hits >= 1):
        final_score = max(final_score, 74)
        classification = "Phishing"
        risk_level = "High"
        trust_score = round(100 - final_score, 2)
    elif classification == "Legitimate" and payment_fraud_hits >= 1 and credential_hits >= 1:
        final_score = max(final_score, 70)
        classification = "Phishing"
        risk_level = "High"
        trust_score = round(100 - final_score, 2)
    elif classification == "Legitimate" and credential_hits >= 2 and any(
        keyword in normalized for keyword in ["account", "mailbox", "kyc", "identity", "payment"]
    ):
    ...
    threat_id = "SM-" + uuid.uuid4().hex[:10].upper()
...
def _insert_scan_record(document: Dict[str, Any]) -> None:
    ...
    if email_collection is None:
        return
    email_collection.insert_one(document)
...
def _update_llm_explanation_by_threat_id(threat_id: str, explanation: str) -> Dict[str, int]:
    with scan_history_lock:
    ...
    logger.warning("Mongo update using threat_id=%s", threat_id)
    ...
    logger.warning(
        "LLM explanation Mongo update completed for threat_id=%s (matched_count=%s, modified_count=%s)",
        threat_id,
        matched_count,
        modified_count,
    )
    if matched_count == 0:
        logger.warning("No matching document found for threat_id")
    ...
    logger.warning(
        "Post-update Mongo readback for threat_id=%s, stored llm_explanation=%s",
        threat_id,
        stored_llm_explanation,
    )
...
async def _generate_and_store_llm_explanation(
    ...
    try:
        ...
        logger.warning("Background threat_id=%s", threat_id)
        ...
        if explanation:
            update_result = await asyncio.to_thread(
                _update_llm_explanation_by_threat_id,
                threat_id,
                explanation,
            )
            logger.warning(
                "LLM explanation store result for threat_id=%s: matched_count=%s modified_count=%s",
                threat_id,
...
def _sync_live_model_state_from_metadata() -> None:
    global current_model_name, model_accuracy, last_retrain_time, next_retrain_time

    model_info_payload = _get_model_info()
    current_model_name = str(model_info_payload.get("model_name") or "N/A")
    model_accuracy = round(float(model_info_payload.get("accuracy", 0.0)) * 100, 2)
    last_retrain_time = str(model_info_payload.get("last_retrain") or "")
    next_retrain_time = str(model_info_payload.get("next_retrain") or "")
...
def retrain_model_from_db() -> Dict[str, Any]:
    global model, current_model_name, model_accuracy, last_retrain_time, next_retrain_time
    ...
    model_accuracy = round(float(results[best_model_name]["accuracy"]) * 100, 2)
    last_retrain_time = retrain_time.isoformat()
    next_retrain_time = (retrain_time + timedelta(minutes=1)).isoformat()
...
def analyze():
    ...
    normalized = normalize_text(full_text)
    guard_urls = re.findall(r"https?://[^\s]+", normalized)
    guard_rule_result = rule_based_score(normalized, guard_urls)

    # Final safety guard: strong phishing language should not return as legitimate.
    if detection.get("classification") == "Legitimate":
        hard_phishing_patterns = [
            r"mailbox\s+is\s+almost\s+full",
            r"revalidate\s+your\s+account",
            r"confirm\s+your\s+identity",
            r"update\s+credentials",
            r"\bkyc\b",
            r"service\s+suspension",
            r"account\s+(?:will\s+be\s+)?suspended",
        ]
        hard_suspicious_patterns = [
            r"confirm\s+payment\s+details",
            r"invoice\s+attached",
            r"pending\s+payment",
        ]

        has_hard_phishing_pattern = any(re.search(pattern, normalized) for pattern in hard_phishing_patterns)
        has_hard_suspicious_pattern = any(re.search(pattern, normalized) for pattern in hard_suspicious_patterns)

        if has_hard_phishing_pattern:
            detection["classification"] = "Phishing"
            detection["risk_level"] = "High"
            detection["trust_score"] = min(float(detection.get("trust_score", 100.0)), 30.0)
        elif has_hard_suspicious_pattern:
            detection["classification"] = "Suspicious"
            detection["risk_level"] = "Medium"
            detection["trust_score"] = min(float(detection.get("trust_score", 100.0)), 45.0)

    if detection.get("classification") == "Legitimate":
        guard_score = int(guard_rule_result.get("score", 0))
        guard_credential_hits = int(guard_rule_result.get("credential_hits", 0))
        guard_threat_hits = int(guard_rule_result.get("threat_hits", 0))
        guard_payment_hits = int(guard_rule_result.get("payment_fraud_hits", 0))
        guard_strong_signal = bool(guard_rule_result.get("strong_signal", False))

        if (
            (guard_strong_signal and guard_score >= 4 and (guard_credential_hits >= 1 or guard_threat_hits >= 1)) or
            (guard_payment_hits >= 1 and guard_credential_hits >= 1)
        ):
            detection["classification"] = "Phishing"
            detection["risk_level"] = "High"
            detection["trust_score"] = min(float(detection.get("trust_score", 100.0)), 30.0)
...
def model_info():
    if "user" not in session:
        return jsonify({"error": "Unauthorized"}), 401
    if current_model_name == "N/A" and model_metadata_collection is not None:
        _sync_live_model_state_from_metadata()
    return jsonify(
        {
