    ...
    container.innerHTML = `
        ...
        <div style="margin-top:22px;">
            ...
            <div id="smartmail-details" style="
                ...
                line-height:1.5;
                opacity:0.9;
            ">${data.explanation_status === "pending"
...
async function pollExplanation(threatId) {
    const maxAttempts = 8;
    const intervalMs = 2000;

    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            const res = await fetch(`${BACKEND_BASE_URL}/analysis/${encodeURIComponent(threatId)}`);
            if (!res.ok) {
                continue;
            }
            ...
            if (payload.explanation_status === "ready" && payload.llm_explanation) {
                const details = document.getElementById("smartmail-details");
                if (details) {
                    details.textContent = payload.llm_explanation;
                }
                return;
            }
        } catch (err) {
            console.error("Explanation polling failed:", err);
        }
...
async function analyzeEmail(email) {
    ...
    try {
        const fullText = email?.text || `${email?.subject || ""}\n\n${email?.body || ""}`;
        ...
        if ((data.explanation_status === "pending" || !data.llm_explanation) && data.threat_id) {
            pollExplanation(data.threat_id);
        }
    } catch (err) {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.action === "injectResult" && message.data) {
        const email = getEmailContent();
        const signature = email ? `${email.subject || ""}${email.body || ""}` : "popup-injected-result";

        injectUI(message.data, signature);
        if ((message.data.explanation_status === "pending" || !message.data.llm_explanation) && message.data.threat_id) {
            pollExplanation(message.data.threat_id);
        }
